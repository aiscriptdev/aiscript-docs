...
if ($env:AISCRIPT_INSTALLER_GHE_BASE_URL) {
  ...
  $installer_base_url = $env:AISCRIPT_INSTALLER_GITHUB_BASE_URL
} else {
...
if ($env:XDG_CONFIG_HOME) {
  ...
  $receipt_home = "${env:LOCALAPPDATA}\aiscript"
}

if ($env:AISCRIPT_DISABLE_UPDATE) {
...
if ($NoModifyPath) {
    Write-Information "-NoModifyPath has been deprecated; please set AISCRIPT_NO_MODIFY_PATH=1 in the environment"
}

if ($env:AISCRIPT_NO_MODIFY_PATH) {
    $NoModifyPath = $true
}

$unmanaged_install = $env:AISCRIPT_UNMANAGED_INSTALL
...
function Install-Binary($install_args) {
  ...
  $platforms = @{
    "aarch64-pc-windows-msvc" = @{
      "artifact_name" = "aiscript-x86_64-pc-windows-msvc.zip"
      "bins" = @("aiscript.exe")
      "libs" = @()
      "staticlibs" = @()
      "zip_ext" = ".zip"
...
function Download($download_url, $platforms) {
  ...
  if (($null -ne $info["updater"]) -and $install_updater) {
    ...
    $out_name = "$tmp\aiscript-update.exe"
...
function Invoke-Installer($artifacts, $platforms) {
  ...
  if (($env:AISCRIPT_INSTALL_DIR)) {
    $force_install_dir = $env:AISCRIPT_INSTALL_DIR
  ...
  if ($install_updater) {
    ...
    [IO.File]::WriteAllLines("$receipt_home/aiscript-receipt.json", "$receipt", $Utf8NoBomEncoding)
  }
