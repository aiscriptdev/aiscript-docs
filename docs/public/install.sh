...
if [ -n "${AISCRIPT_INSTALLER_GHE_BASE_URL:-}" ]; then
    ...
    INSTALLER_BASE_URL="${AISCRIPT_INSTALLER_GITHUB_BASE_URL:-https://github.com}"
fi
...
if [ -n "${AISCRIPT_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$AISCRIPT_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${AISCRIPT_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${AISCRIPT_UNMANAGED_INSTALL:-}"
...
download_binary_and_run_installer() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set AISCRIPT_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
    ...
    case "$_artifact_name" in 
        "aiscript-aarch64-apple-darwin.tar.xz")
            ...
            _updater_bin=""
            ;;
        "aiscript-x86_64-apple-darwin.tar.xz")
            ...
            _updater_bin=""
            ;;
        "aiscript-x86_64-pc-windows-msvc.zip")
            ...
            _updater_bin=""
            ;;
        "aiscript-x86_64-unknown-linux-gnu.tar.xz")
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-apple-darwin")
            _archive="aiscript-aarch64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="aiscript-x86_64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "aarch64-pc-windows-msvc")
            _archive="aiscript-x86_64-pc-windows-msvc.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "x86_64-pc-windows-msvc")
            ...
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="aiscript-x86_64-unknown-linux-gnu.tar.xz"
...
install() {
    ...
    if [ -n "${AISCRIPT_INSTALL_DIR:-}" ]; then
        _force_install_dir="$AISCRIPT_INSTALL_DIR"
